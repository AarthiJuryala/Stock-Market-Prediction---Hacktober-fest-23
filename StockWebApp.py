...
if page == "Stock Trends Exploration":
    ...
    if start_date and end_date and symbol:
        ...
        for target in ['open', 'close', 'volume']:
            ...
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=forecast['ds'], y=forecast['yhat'], name=f'Forecast {target.title()}'))
...
if page == "Compare Stocks":
    ...
    if stock1 and stock2:
        df1 = load_twelvedata(stock1, '2022-06-01', '2023-06-01')
        df2 = load_twelvedata(stock2, '2022-06-01', '2023-06-01')

        for column, label in zip(['Open', 'Close', 'Volume'], ['Opening Price', 'Closing Price', 'Volume']):
            fig, ax = plt.subplots(figsize=(12, 6))
            fig.set_facecolor('black')
            ax.set_facecolor('black')
            ax.plot(df1.index, df1[column], label=f'{comp_keys[stock1]} {label}')
            ax.plot(df2.index, df2[column], label=f'{comp_keys[stock2]} {label}')
